...
strategy = st.sidebar.selectbox(
    "Strategy",
    ["Prudent", "Normal", "Aggressive"]
)

nb_features_explain = st.sidebar.slider(
    label='Number of features to explain decision',
    min_value=1,
    max_value=20,
    value=5,
    step=1
...
st.title("Home Credit Decision Making Tool")
...
st.header(
    f"With {article} {strategy.lower()} strategy, the model {litteral_decision.upper()} the client"
)


col_score, col_explanation = st.columns([2,1])
...
with col_explanation:
    ...
    explanation_client.sort_values('shap_value', ascending=ascending_bool, inplace=True)
    ...
    st.dataframe(explanation_client)
